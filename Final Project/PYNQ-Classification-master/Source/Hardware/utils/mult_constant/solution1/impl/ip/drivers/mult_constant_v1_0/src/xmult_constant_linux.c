...
typedef struct {
    ...
    XMult_constant_uio_map maps[ MAX_UIO_MAPS ];
} XMult_constant_uio_info;

/***************** Variable Definitions **************************************/
static XMult_constant_uio_info uio_info;
...
static int uio_info_read_name(XMult_constant_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMult_constant_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMult_constant_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMult_constant_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMult_constant_Initialize(XMult_constant *InstancePtr, const char* InstanceName) {
	XMult_constant_uio_info *InfoPtr = &uio_info;
